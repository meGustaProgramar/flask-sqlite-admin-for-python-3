...
class sqliteAdminFunctions:
	...
	def __init__(self,dbPath,tables=[],extraRules=[]):
		self.dbPath = dbPath
	...
	def tableContents(self,table,sort,dir,offset):
		...
		if table in self.tables:
			...
			if res['schema'][0]['primaryKey'] == 1:
				...
				cur = con.cursor()
				c = cur.execute('select count(?) as c from %s' % (table),[res['primaryKey']])
				res['count'] = c.fetchone()['c']
		
				if sort == '': sort = res['primaryKey']
				l = cur.execute('select * from %s order by %s %s limit ?,50' % (table,sort,dir),[int(offset)*50])
	...
	def tableSchemas(self,table):
		...
		rows = cur.fetchall()
		return [{'name':row[1],'dataType':row[2],'notNull':row[3],'primaryKey':row[5]} for row in rows]
